...
void DvProcessDracula(DraculaView dv, char *move) {
	...
	if (move[1] == 'D') {
		...
		currLoc = CityIdFromMove(dv->PastPlaysArray[dv->turnCounter - DBval*5]);
	...
	if (action[2] == 'M') {
		...
		RemoveTrapFromLoc(CityIdFromMove(dv->PastPlaysArray[dv->turnCounter - 30]), dv->map);
	} else if (action[2] == 'V') {
		// Mature Vamp
		DvMatureVampire(dv);
	}
